...
- [x] Обновить `CLAUDE.md` секцию архитектуры — добавить упоминание GigaAM как четвёртого локального провайдера
- [x] Переместить этот план в `docs/plans/completed/`
- [x] (Опционально, для прод-полировки — вне scope демки) добавить `LICENSE-GigaAM` (MIT) и упоминание в About (skipped — явно вне scope демки)
